...
GITHUB_DATASET_URL = "https://github.com/saimasano123/NewsBot-Intelligence-System-NLP-Midterm-Project/blob/main/data/bbc_news_train.csv"
...
print(f"📍 Attempting to load dataset from GitHub...")
...
try:
    ...
    if "YOUR_GITHUB" not in GITHUB_DATASET_URL:
        ...
        print("⚠️  GitHub URL not configured yet!")
    ...
    print("\n🔄 Alternative: Upload file manually")
    from google.colab import files
...
print(f"   • Original: {df_original_size} articles")
print(f"   • After cleaning: {len(df)} articles")
print(f"   • Removed: {df_original_size - len(df)} articles")
...
business_context = """
🎯 APPLICATION: NewsBot Intelligence System for Media Monitoring
...
print(f"📊 Dataset: {len(df)} articles across {len(df['category'].unique())} categories")
print(f"📁 Categories: {df['category'].unique().tolist()}")

# Apply preprocessing
print("\n🔄 Preprocessing articles (this may take 1-2 minutes)...")
...
print(f"   • Average original length: {avg_length_original:.0f} characters")
print(f"   • Average processed length: {avg_length_processed:.0f} characters")
...
print(f"✅ TF-IDF Matrix created: {X_tfidf.shape}")
...
# Visualize top terms
print("\n📊 Generating TF-IDF visualization...")
...
print("🔍 Analyzing POS patterns (sampling 100 articles per category)...")
sample_size = min(100, len(df))
df_sample = df.groupby('category').head(20).reset_index(drop=True)

print(f"   • Analyzing {len(df_sample)} articles...")
...
print("🔍 Extracting dependency patterns from sample articles...")
sample_patterns = []
...
print(f"\n📝 Sample Dependency Patterns (showing 15):")
print(f"{'='*70}")
...
print("🔍 Extracting named entities (this may take 2-3 minutes)...")
entity_results = []
sample_size = min(100, len(df))
